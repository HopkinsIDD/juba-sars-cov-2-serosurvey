...
p_delta <- 0.99
n_treedepth <- 20
stan_redo <- FALSE # set this to TRUE to re-run the Stan models, FALSE to load previous model fits
...
## Supplemental analysis: seroprevalence and risk of seropositivity within additional age groups

```{r epi data with addl age groups}
# Juba age data
ssd_ages <- fread(here::here('data', 'raw_data', 'population_data', 
                             'ssd_urban_age_sex.csv'))
ssd_ages[, men := sum(men/100*510000), by = age_cat]
ssd_ages[, women := sum(women/100*510000), by = age_cat]
ssd_ages <- unique(ssd_ages[, c('age_cat2', 'men', 'women')])
setnames(ssd_ages, 'age_cat2', 'age_cat')
...
```{r stan for addl age groups}
# set arguments
run_id <- 'mild80-addl-ages'
severity_probs <- severity_groups[['mild80']]
message('Running analysis: ', run_id)
...
# overall seroprevalence by age and sex
age_sex_sero_addl <- subset_est[var == 'Age' | var == 'sex']
age_sex_sero_addl[, mean := mean(p, na.rm = T), by = 'val']
age_sex_sero_addl[, lower := quantile(p, probs = .025, na.rm = T), by = 'val']
age_sex_sero_addl[, upper := quantile(p, probs = .975, na.rm = T), by = 'val']
age_sex_sero_addl <- unique(age_sex_sero_addl[, c('val', 'mean', 'lower', 'upper')])

# relative risk by age and sex
rr_by_age_addl <- get_rr_by_group(subset_est, age_ref = '[20,30)')

# save results if these are a new model run
if (stan_redo) {
  write.csv(rr_by_age_addl, here::here('data', 'generated_data', 'adjusted_prev',
                                paste0('rr_by_ages_addl_', Sys.Date(), '.csv')))
  write.csv(age_sex_sero_addl, here::here('data', 'generated_data', 'adjusted_prev',
                                       paste0('age_sex_seroprev_addl_', Sys.Date(), '.csv')))
}
```
...
## Table S2. Relative risk and seropositivity by additional age groups

```{r}
# relative risk
rr_tab2 <- data.table(rr_by_age_addl)
rr_tab2[, row_order := c(1, 3, 4, 5, 6, 2, 7, 8, 9, 10)]
setorderv(rr_tab2, 'row_order')
rr_tab2[, row_order := NULL]
rr_tab2[, Category := c('1 - 4 years', '5 - 9 years', '10 - 19 years', 
                        '20 - 29 years', '30 - 39 years', '40 - 49 years',
                        '50 - 64 years', '65 - 84 years', 'Female', 'Male')]

# seropositivity
age_sex_sero2 <- copy(age_sex_sero_addl)
cols <- c('mean', 'lower', 'upper')
age_sex_sero2[, row_order := c(9, 10, 1, 3, 4, 5, 6, 2, 7, 8)]
setorderv(age_sex_sero2, 'row_order')
age_sex_sero2[, row_order := NULL]
age_sex_sero2[, (cols) := lapply(.SD, function(x) round(x, 3)*100), .SDcols = cols]
age_sex_sero2[, `Seroprevalence (95% CrI)` := paste0(mean, ' (', lower, ' - ', upper, ')')]
age_sex_sero2[, (cols) := NULL]

# combine
tab3 <- cbind(rr_tab2[, 2:5], age_sex_sero2[, 2], rr_tab2[, 6])

# table
table3 <- flextable(tab3) %>%
  autofit() %>%
  valign(valign = 'top')

save_as_docx(table3, path = paste0(outdir, '/tableS2.docx'))
```
